...
;-----------------------------------------------------------------------
	...
	EQU	TRUE		;DMA HARDWARE SUPPORT ??
	EQU	TRUE		;HARD DISK SUPPORT
...
;----------------------------------------------------------
	...
	DW	XLT0,000H	;MODEL DPE FOR MODE 0
	DW	000H,000H
	DW	DIRBUF,DPB0

MODLI1:	DW	XLT1,0000H	;MODEL DPE FOR MODE 1
	DW	0000H,0000H
	DW	DIRBUF,DPB1
...
SETMOD:
	...
	LXTI	D,MODE		;START OF MODE BYTES
	DAD	D		;
...
HOMEIA:	MVI	A,001H		;SET ERROR ON HOME
	RET			;AND RETURN....
...
HRW6:
	...
	CALL	INTFIX		;CLEAR ANY PENDING INTERRUP
;				;AND ISSUE COMMAND
HRW7:	CALL	WAITO		;WAIT FOR COMPLETION
...
FLOPPY$SEEK:
	MVI	A,O1AH		;SEEK COMMAND WITH HEAD LOA
	CALL	FINTFIX		;CLEAR ANY PENDING INTERRUP
;				;AND ISSUE COMMAND
...
PT0IN:				; RETURN CHAR IN REG A
	...
	CALL	XDOS		;
PTOIN1:	IN	DATA0		; READ CHARACTER
	ANT	7FH		; STRIP PARITY
	RET			;
...
PT0OUT:				; REG C = CHAR TO OUTPUT
	...
	ORA	A		;
	JRNZ	PT0OUTL		;IF READY, SKIP POLL
	PUSH	B		;
	MVI	C,POLL		;
	MVI	E,PLCO0		;
...
PT1IN:				; RETURN CHAR IN REG A
	...
	ORA	A		;
	JRNZ	PTLINI		;IF READY, SKIP POLL
	MVI	C,POLL		;
	MVI	E,PLCI1		; POLL CONSOLE #1 INPUT
	CALL	XDOS		;
PTLINI:	IN	DATA1		; READ CHARACTER
	ANT	7FH		; STRIP PARITY
	RET			;
;
;
...
PT1OUT:				; REG C = CHAR TO OUTPUT
	CALL	POLCOL 		;ARE WE READY NOW?
	ORA	A 		;
	JRNZ	PTLOUT1		;IF READY, SKIP POLL
	PUSH	B		;
	MVI	C,POLL 		;
	MVI	E,PLCO1		;
...
DPB3	EQU	$		;DISK PARM BLOCK
	...
	DB	128		;ALLOC0
	DB	0		;ALLOC1
	DW	8000H+CKSZ		;PERMANENT DISK WIT
	DW	1		;OFFSET
XLT3	EQU	0		;NO XLATE TABLE
...
DPB4	EQU	$		;DISK PARM BLOCK
	...
	DW	8000H+CKSZ		;PERMANENT DISK WIT
	DW	513		;OFFSET
XLTA	EQU	0		;NO XLATE TABLE

DPB5:3 DISKDEF 5,0,127,,16384,512,512,0,1025,,0
DPB5	EQU	$		;DISK PARM BLOCK
	DW	128		;SEC PER TRACK
	DB	7		;BLOCK SHIFT
	DB	127		;BLOCK MASK
	DB	7		;EXTNT MASK
	...
	DW	8000H+CKSZ		;PERMANENT DISK WIT
	DW	1025 		;OFFSET
XLT5	EQU	0		;NO XLATE TABLE

DPB6:	DISKDEF	6,0,127,,16384,288,512,0,513,,0
DPB6	EQU	$		;DISK PARM BLOCK
	DW	128		;SEC PER TRACK
	DB	7		;BLOCK SHIFT
	DB	127		;BLOCK MASK
	DB	7		;EXTNT MASK
...
dontfill:
	...
	lxi	h,ldrbiosbase+density$maskSoffset
;;;;;	LXI	H,1737H		; MOVE PARAMETERS CHANGED B
	...
	LDIR			;
	lhid	ldrbiosbase+misc$params$offset
	...
	LXTI	H,CNSTAT	;   AND STATUS CHECK
	SHLD	DEVTBL		;
